...
class DenotationGraph(object):
    def __init__(self, graph_folder_path: str) -> None:
    ...
    def get_subgraph_from_edges(self, edges: List[Tuple[int, int]]) -> nx.DiGraph:
        return self.graph.edge_subgraph(edges)

    def get_subgraph_from_nodes(self, nodes: List[int]) -> nx.DiGraph:
    ...
    def _create_graph(self) -> None:
        ...
        for idx, images, captions in tqdm(
            zip(self._node_idx, self._node_images, self._node_captions),
            total=len(self._node_idx),
            ...
            assert captions[0] == i
            captions = captions[1:]
            self.graph.add_node(int(i), text=text, images=sorted(images), captions=sorted(captions))

        for n in tqdm(self._node_tree, desc="Adding edges to the graph..."):
            if n == "":
                continue
            ...
            self.graph.add_edge(parent, child, edge_type=edge, rewrite_rules=rewrite_rules)
